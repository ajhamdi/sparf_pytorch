...
class CO3DDataset(DatasetBase):
    ...
    def load_sequence(self, sequence_id: int):
        ...
        for i in tqdm(range(self.curr_offset, self.next_offset)):
            ...
            ################################3
            mask_path, masks_name = os.path.split(self.image_path[i])
            masks_name = masks_name[:-3]+"png"
            mask_path = os.path.split(mask_path)[0]
            mask_path = path.join(self.data_dir, mask_path, "masks", masks_name)
            mask = (cv2.imread(mask_path)[:,:,0]>100.0).astype(np.float32)

            #######################
            im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0

            im = im[..., :3]
            im = im*mask[..., None] #+ (1.0 - mask[..., None])  # remove BG
            h, w, _ = im.shape
            max_hw = max(h, w)
            ...
            if approx_scale < 1.0:
                ...
                mask = cv2.resize(mask, (w2, h2), interpolation=cv2.INTER_AREA)
